...
const Portfolio = () => {
    ...
    const { data, isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            // 💡 Note: Local json file run korar jonno pather age '/' dewa bhalo
    ...
    return (
        <div id="portfolio" className="text-white container mx-auto p-5 md:p-5">
            ...
            <div className="grid grid-cols-1 justify-center mb-10 mt-10 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {
                    // 💡 Ekhane check kore newa hochhe 'data' shotti array kina, na hole crash korbe na
                    Array.isArray(data) ? (
                        data.slice(0, 3).map(project => (
                            <ProjectCard key={project.id} project={project}></ProjectCard>
                        ))
                    ) : (
                        <p className="text-red-500 text-center col-span-full">Data is not in correct format or file not found!</p>
                    )
                }
            </div>
